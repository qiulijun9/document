# http 是什么
http 是超文本传输协议
超文本：除了文本之外，还包含图像，音频，视频，超链接，能够从一个超文本链接到另一个超文本，形成一个复杂的网状关系。

所以说http 是在专门的两点之间传输文本，图像，音频，视频，等超文本数据的规范和约定
# http 特点
...
- 头部字段（集合）：使用key-value的形式说明报文
  请求头/响应头
  属性：值
  Host: 127.0.0.1:10000
  Connection: keep-alive

- 空行 ：空行标识空行之后的内容都为消息正文
- 消息正文：代表请求或响应的内容
...
3. 专用的头部压缩算法，采用 HPACK 压缩算法，压缩头部，减小传输的体积
4. 支持服务端推送，是指把客户端所需要的 css,js，img 资源伴随着 index.html 一起发送到客户端，客户端从缓存中取

https://mp.weixin.qq.com/s/7tKgSmkMWHdIyvrTWJE8sg

## http2 支持的二进制传送是指？
http1.1的请求和响应都是由起始行 + 头部字段 + 空行 + 消息正文等组成（Header+Body的形式），
http2 将请求和响应数据分割为更小的二进制帧，采用二进制进行传送，方便了计算机解析，体积小，速度快。
...
- 帧头
  帧长度：表示帧体的长度
  帧类型：分为数据帧和控制帧，数据帧用来存储 http 报文，控制帧用来管理流的传输
  标志位：一共 8 个标志位， 常用的有 END_HEADERS 表示头数据结束，END_STREAM 表示单方向数据发送结束。
  流标识符：服务器端根据该标识，相同 id 的二进制组成请求/响应报文
- 帧体：代表实际传输的数据

## 虚拟的  流
流是二进制双向传输的序列，在里面流动的先后顺序的数据帧，组装起来就是http1 里的请求和响应的报文。

http2 的多路复用---是指多个往返通信都复用一个连接来处理，解决了对头阻塞的问题
