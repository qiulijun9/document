/**
 * splice 
	删除：删除数组某一项(会改变原数组) splice(index,删除个数，[添加元素可选])
	插入： splice(index,0,要插入的元素可以任意个)
  替换： splice(index,删除元素的个数,插入元素可任意多个)
	返回值：返回从数组中删除的元素，若没有删除的元素，返回空数组
 * 
 *  */
...
let arr2 = [1, 2, 3, 4]
arr2.splice(1, 1, 5) //1,5,3,4
arr2.splice(1, 0, 'red', 'blank') // [1, 'red', 'blank', 5, 3, 4 ]
console.log(arr2)
...
//判断数组中是否包含某个值 以下方法都接收两个参数（查找的元素，可选的开始搜索的位置下标）
//indexOf() ,如果存在返回元素下标，否则返回-1，实现原理也是通过for循环来返回下标值
//lastIndexOf(),从数组末尾向前搜索,返回元素下标，不存在返回-1
//includes(),是否包含某个元素，从数组的首部开始搜索 返回true 或false
let arr4 = [3, 4, 5, 6, 7, 7]
console.log(arr4.indexOf(7)) //4
console.log('lastIndexOf', arr4.lastIndexOf(7)) //5
// includes 如果存在返回true,否则返回false
